window.addEventListener("load",function(){
  ...
  const inputTel3 = document.getElementById('tel-3');//＝電話番号の入力欄
  if (!inputTel3){ return false;}
  //電話番号の入力欄から離れたら発動
  inputTel3.addEventListener('blur', () => {
    // バリデーション関数
    let validateTelNeo = function (value) {
      return /^[0０]/.test(value) && libphonenumber.isValidNumber(value, 'JP');
    }
    ...
    const postdata = inputTel3.value;//入力した電話番号を取得
    //入力した内容がバリデーションに引っかかったときはエラー
    if (!validateTelNeo(postdata)) {
      console.log('ERROR')
      return
    ...
    let formattedTel = formatTel(postdata);//入力した電話番号を整形
    console.log(formattedTel);
    inputTel3.value = formattedTel;//整形した電話番号を入力欄に返す
  });
});